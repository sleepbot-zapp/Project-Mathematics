...
from tools import convert_latex, return_latex_surface, solve
    ...
    def __init__(self, surface, width, height):
        ...
        self.input_box = pygame_gui.elements.UITextEntryLine(
            relative_rect=pygame.Rect(
                (((self.WIDTH - (self.WIDTH // 2)) // 2), 100), (self.WIDTH // 2, 50)
            ),
        ...
        self.solve_button = pygame_gui.elements.UIButton(
            relative_rect=pygame.Rect(
                (((self.WIDTH - (self.WIDTH // 2)) // 2), 160), (self.WIDTH // 2, 50)
            ),
            text="Calculate",
            manager=self.manager,
        )
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.VIDEORESIZE:
                    ...
                    self.input_box.set_position(
                        (((self.WIDTH - (self.WIDTH // 2)) // 2), 100)
                    )
                    self.input_box.set_dimensions((self.WIDTH // 2, 50))
                    self.solve_button.set_position(
                        (((self.WIDTH - (self.WIDTH // 2)) // 2), 160)
                    )
                ...
                if event.type == pygame.USEREVENT:
                    if event.user_type == pygame_gui.UI_BUTTON_PRESSED:
                        try:
                            if (
                                event.ui_element == self.solve_button
                                and self.input_box.get_text().strip() is not None
                            ):
                                self.eq_solve()
                                self.result_surf = return_latex_surface(
                                    convert_latex(self.equation)
                                )
                        except Exception as e:
