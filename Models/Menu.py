...
class Menu:
    ...
    def __init__(self, width, height, surface):
        self.img_directory = (Path(__file__).resolve().parent.parent / 'Images' / 'Menu.png').as_posix()   
        ...
        self.func_lists = ['Graph', 'Eq_Solver', 'Derivative', 'Integral', 'Extrema', 'Agents']
        self.running=True 
    @property
    def FONT(self):
    ...
    def cords(self):
        ...
        self.boxes = [pg.Rect(self.total_boxes_x, self.total_boxes_y,self.total_boxes_x*6, self.total_boxes_y), 
                      pg.Rect(self.total_boxes_x*8, self.total_boxes_y, self.total_boxes_x*6, self.total_boxes_y),
                      pg.Rect(self.total_boxes_x, self.total_boxes_y*3,self.total_boxes_x*6, self.total_boxes_y), 
                      pg.Rect(self.total_boxes_x*8, self.total_boxes_y*3, self.total_boxes_x*6, self.total_boxes_y),
                      pg.Rect(self.total_boxes_x, self.total_boxes_y*5,self.total_boxes_x*6, self.total_boxes_y), 
                      pg.Rect(self.total_boxes_x*8, self.total_boxes_y*5, self.total_boxes_x*6, self.total_boxes_y),]        
        return self.boxes
    @property
    ...
    def run(self):
        while self.running:
            ...
            for i in self.cords:
                ...
                text = self.FONT.render(self.func_lists[self.cords.index(i)], True, (0, 0, 0))
